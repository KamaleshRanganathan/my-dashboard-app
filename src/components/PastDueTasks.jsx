...
export default function PastDueTask({ tasks, onDelete, onUpdateStatus }) {
  ...
  return (
    <>
      ...
      <div id="content4" style={{ display: isHidden ? "block" : "none" }}>
        {tasks
          .filter(
            (task) =>
              new Date(task.deadline) < new Date() && task.status !== "Yes",
          )
          .map((task) => (
            <div key={task.id} className="flex p-2 w-6/8 m-auto ">
              ...
              <div className="border border-neutral p-2 max-w-1/4 h-auto my-2 ml-2 flex items-center justify-center">
                ...
                >
                  <img src="src/assets/tick.svg" alt="Mark as Not Done" />
                </button>
                <button
                  className="btn flex items-center justify-center bg-base-100 border-transparent shadow-none rounded-sm"
